...
コメントの行番号は以下のルールに従ってください：
- **Diff内に表示されている行**のみにコメントできます
- 行番号は「+」または「空白（コンテキスト行）」で始まる行の、新しいファイルでの行番号です
- 「-」で始まる削除行にはコメントできません
- Diffに表示されていない行（省略された行）にはコメントできません
- 各hunkのヘッダー「@@ -X,Y +A,B @@」の「A」が新ファイルの開始行番号です

例：
\`\`\`diff
@@ -10,5 +10,7 @@
 context line  // line 10 - コメント可
+new line      // line 11 - コメント可（追加行）
-deleted line  // コメント不可（削除行）
 context line  // line 12 - コメント可
\`\`\``;
