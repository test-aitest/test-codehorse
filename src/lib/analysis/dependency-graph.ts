    ...
    for (const imp of imports) {
      if (!visited.has(imp.targetFile)) {
        ...
        if (cycleStart !== -1) {
          ...
          if (!cycles.some(c =>
            normalizeCycle(c.cycle).join("->") === normalizedCycle.join("->")
          )) {
            cycles.push({
              cycle,
...
export function getGraphStats(graph: DependencyGraph): {
  ...
  for (const node of graph.nodes.values()) {
    const depCount = node.imports.length;
    totalDependencies += depCount;
    maxDependencies = Math.max(maxDependencies, depCount);

    if (depCount === 0) filesWithNoDependencies++;

    const importerCount = graph.reverseIndex.get(node.filePath)?.length || 0;
    if (importerCount === 0) filesWithNoImporters++;
  }

  return {
    totalFiles: graph.nodes.size,
    totalDependencies,
    avgDependencies: graph.nodes.size > 0 ? totalDependencies / graph.nodes.size : 0,
    maxDependencies,
