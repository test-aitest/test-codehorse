...
exports.parseTestUpdatesFromClaudeOutput = parseTestUpdatesFromClaudeOutput;
exports.normalizeTestCase = normalizeTestCase;
exports.generateNextTestCaseId = generateNextTestCaseId;
/**
 ...
 * ```
 */
function parseTestUpdatesFromClaudeOutput(output) {
    if (!output)
        return [];
    ...
    while ((match = jsonBlockPattern.exec(output)) !== null) {
        try {
            ...
            console.log(`[TestCaseParser] Failed to parse JSON block: ${e.message}`);
        }
    }
    if (updates.length > 0) {
        return updates;
 ...
 */
function isValidTestCaseUpdate(obj) {
    if (typeof obj !== "object" || obj === null)
        return false;
    const update = obj;
    // action のチェック
    if (!["add", "update", "delete"].includes(update.action)) {
        return false;
    }
    // testCase のチェック
    const testCase = update.testCase;
 ...
 * 既存のIDの最大値 + 1 を使用
 */
function generateNextTestCaseId(existingTestCases) {
    const pattern = /^TC(\d+)$/;
    let maxNum = 0;
