...
function testJSONRepairEdgeCases() {
  ...
  const nestedResult = repairAndParseJSON(nestedJSON, ReviewResultSchema);
  ...
  const unicodeResult = repairAndParseJSON(unicodeJSON, ReviewResultSchema);
  ...
  const emptyFieldsResult = repairAndParseJSON(emptyFieldsJSON, ReviewResultSchema);
  ...
  const multilineResult = repairAndParseJSON(multilineJSON, ReviewResultSchema);
  logTest("JSON", "改行含むJSON: パース成功", multilineResult.success === true);

  // 不正なプロパティ名（修復不可）
  const invalidPropJSON = "{ invalid-prop: 'value' }";
  const invalidPropResult = repairAndParseJSON(invalidPropJSON, ReviewResultSchema);
...
function testMemoryLimits() {
  ...
  const parseResult = repairAndParseJSON(largeJSON, ReviewResultSchema);
...
function testErrorRecovery() {
  ...
  for (let i = 0; i < brokenJSONs.length; i++) {
    const result = repairAndParseJSON(brokenJSONs[i], ReviewResultSchema);
    logTest("リカバリー", `壊れたJSON ${i + 1}: エラーハンドリング`, result.attempts.length > 0);
  }
}
...
function testReportGeneration() {
  ...
  // 修復サマリー
  const repairSummary = formatRepairSummary({
    success: true,
    data: { summary: "test", walkthrough: [], comments: [] },
    attempts: [
      { strategy: "direct_parse", success: false, error: "Failed" },
      { strategy: "extract_markdown", success: true },
    ],
  });
  logTest("レポート", "修復サマリー: 生成成功", repairSummary.length > 0);
  logTest("レポート", "修復サマリー: 戦略含む", repairSummary.includes("extract_markdown"));
